...
<?php
    ...
    function importDatabase($pt)
    {
        try {
          ...
          echo "<div class='alert alert-success'>Database Connected successfully".'</div>';
            $query = file_get_contents("./lib/database.sql");
    ...
    function setAdminEmail($pt){
        $db = new PDO("mysql:host=$pt[database_host];dbname=$pt[database_name]", $pt['database_username'], $pt['database_password']);
        $res = $db->query("UPDATE users SET email='".$pt['admin_email']."' WHERE username='admin'");
        if ($res){
            return true;
        }else{
            return false;
    ...
    <div class="get-support">
        <div class="icon-wrap">
            <ul class="support-list">
                <li><a href="http://rkixtech.com/">Visit Website</a></li>
                <li><a href="mailto:contact@rkixtech.com">Contact Support</a></li>
            </ul>
                            ...
                            <table class="requirement-check">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        $database_errors = 0;
                                        $database = file_exists('./lib/database.sql');
                                        if ($database==true) {
                                            print '<div class="icon check"><i class="fa fa-check-circle" aria-hidden="true"></i></div>';
                                        }else{
                                            $database_errors++;
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="copyright-inner">
                    &copy; 2021 All Right Reserved By <a href="https://rkixtech.com/">Rkixtech</a>
                </div>
            </div>
        </div>
    </div>
