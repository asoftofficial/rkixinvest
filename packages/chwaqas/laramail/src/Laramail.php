...
use Chwaqas\Laramail\Utils\Replacer;
...
class Laramail
{
    public const VIEW_NAMESPACE = 'laramail';
    ...
    {
        $file = config('laramail.mailables_dir').'templates.json';
        if (! file_exists($file)) {
            if (! file_exists(config('laramail.mailables_dir'))) {
                if (! mkdir($concurrentDirectory = config('laramail.mailables_dir')) && ! is_dir($concurrentDirectory)) {
    ...
    {
        if (config('laramail.relation_depth') === 0) {
            return $factoryModel;
        }

        if (self::$traversed >= 5) {
            Log::warning('[Laramail]: more than 5 calls to relation loader', ['last_model' => get_class($factoryModel) ?? null]);
            self::$traversed = 6;
    ...
    public static function loadRelations($relationName, $factoryModel, $eloquentFactory = null): ?object
    {
        try {
            ...
            if (is_null($factoryModel->$relationName) || $loadIfIterable) {
                ...
                if (self::$traversed <= config('laramail.relation_depth')) {
                    if (! $loadIfIterable) {
