...
class LaramailMailables
{
    ...
    public static function all()
    {
        ...
        if (! file_exists(config('laramail.mailables_dir'))) {
            return;
        } else {
            $allFiles = new RecursiveIteratorIterator(new RecursiveDirectoryIterator(config('laramail.mailables_dir')));
            ...
            foreach ($phpFiles as $phpFile) {
                ...
                for ($index = 0; isset($tokens[$index]); $index++) {
                    ...
                    if (T_CLASS === $tokens[$index][0] && T_WHITESPACE === $tokens[$index + 1][0] && T_STRING === $tokens[$index + 2][0]) {
                        ...
                        if (! Laramail::mailable_exists($mailableClass)) {
                            continue;
                        }
                        ...
                        $mailable_data = Laramail::buildMailable($mailableClass);

                        if (! is_null(Laramail::handleMailableViewDataArgs($mailableClass))) {
                            $mailable_view_data = Laramail::getMailableViewData(Laramail::handleMailableViewDataArgs($mailableClass), $mailable_data);
                        } else {
                            $mailable_view_data = Laramail::getMailableViewData(new $mailableClass, $mailable_data);
                        }

                        $fqcns[$i]['data'] = $mailable_data;
                        $fqcns[$i]['markdown'] = Laramail::getMarkdownViewName($mailable_data);
                        $fqcns[$i]['name'] = $name;
